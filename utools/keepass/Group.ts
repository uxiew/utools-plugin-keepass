...
import { T_Group } from "../../src/typings/data";
...
export class Group {
  ...
  geState(): T_Group {
    ...
    return ({
      ...
      notes: "",
      items: this.items.map(g => g.geState()),
      expanded: this.expanded,
      parentId: this.parentGroup?.id
    })
  }

  private _iconFromId(id: number = -1) {
  ...
  private _buildCustomIcon() {
    this.customIcon = null;
    if (this._group.customIcon) {
      return IconUrlFormat.toDataUrl(
        this.file.db.meta.customIcons.get(this._group.customIcon.id)?.data
      );
    }
    return null;
  }

  private _groupModified() {
  ...
  private _fillByGroup() {
    this.title = this._group.name || this.file.name
    this.iconId = this._group.icon!
    this.icon = this._iconFromId(this._group.icon)
    this.customIcon = this._buildCustomIcon()
    this.customIconId = this._group.customIcon ? this._group.customIcon.toString() : null
    this.expanded = this._group.expanded !== false
  }
