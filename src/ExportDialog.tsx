...
export default class ExportDialog extends Component {
  ...
  handlePasswordChange = (e: SyntheticEvent<Element, Event>) => {
    this.setState({ password: e.target.value });
  };

  handleSubCheckeckChange = (e: SyntheticEvent) => {
  ...
  handleExport = () => {
    ...
    accountList.forEach((x) => {
      let content =
        '【' + window.services.decryptValue(keyIV, x.title) + '】\n';
      content +=
        '用户名：' + window.services.decryptValue(keyIV, x.username) + '\n';
      content +=
        '密码：' + window.services.decryptValue(keyIV, x.password) + '\n';
      if (x.link) {
        content +=
          '链接：' + window.services.decryptValue(keyIV, x.link) + '\n';
      }
      if (x.remark) {
        content +=
          '说明：' + window.services.decryptValue(keyIV, x.remark) + '\n';
      }
    ...
    return (
      <Dialog open={open} onClose={this.handleClose}>
        ...
        <DialogContent className='export-dialog-content'>
          ...
          />
          <FormControlLabel
            onChange={this.handleSubCheckeckChange}
            checked={subChecked}
            control={<Checkbox />}
            label='包含子分组帐号数据'
          />
